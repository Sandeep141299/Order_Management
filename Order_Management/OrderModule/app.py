...
def otm():
    ...
    joining_date_conv = datetime.datetime.strptime(Otm_joiningDate, "%Y-%m-%d")
    print(Otm_contact)
    ...
    userInput = (Otm_username, Otm_lastname,
                 Otm_password, Otm_email, Otm_contact, joining_date_conv, Level)
    result = cursor.execute(insertQuery, userInput)
    mysql.connection.commit()
...
def addtask():
    ...
    Start_date_conv = datetime.datetime.strptime(Start_date, "%Y-%m-%d")
    End_date = flask.request.form['End_date']
    End_date_conv = datetime.datetime.strptime(End_date, "%Y-%m-%d")
    Expert_startDate = flask.request.form['Expert_startDate']
    Expert_startDate_conv = datetime.datetime.strptime(
        Expert_startDate, "%Y-%m-%d")
    Expert_endDate = flask.request.form['Expert_endDate']
    Expert_endDate_conv = datetime.datetime.strptime(
        Expert_endDate, "%Y-%m-%d")
    ...
    print(Task_Subject, Vendor_budget,
          Expert_firstname, Client_name, Status, Start_date_conv, End_date_conv, Expert_startDate_conv, Expert_endDate_conv, Qc_Expert_name, Otm_username, Description, Word_count, Expert_price)
...
def getotm1():
    ...
    # print(listit(data))
    respp.set_data(json.dumps(jsondata))
...
def updateotm(userId):
    ...
    userInput = (Otm_username, Otm_lastname,
                 Otm_password, Otm_email, Otm_contact, joining_date_conv, Level, userId)
...
def updatetask(userId):
    ...
    print(Task_Subject)
    Client_name = flask.request.form['Client_name']
    print(Task_Subject, "4")
    Status = flask.request.form['Status']
    print(Task_Subject, "5")
    Start_date = flask.request.form['Start_date']
    print("======", Start_date)
    Start_date_conv = datetime.datetime.strptime(Start_date, "%Y-%m-%d")
    print(Start_date_conv, "6")
    End_date = flask.request.form['End_date']
    End_date_conv = datetime.datetime.strptime(End_date, "%Y-%m-%d")
    Expert_startDate = flask.request.form['Expert_startDate']
    Expert_startDate_conv = datetime.datetime.strptime(
   ...
   # Update
    ...
    userInput = (Task_Subject, Vendor_budget,
                 Expert_id, Client_id, Status, Start_date_conv, End_date_conv, Expert_startDate_conv, Expert_endDate_conv, Qc_Expert_id, Otm_id, Description, Word_count, Expert_price, userId)
...
# @app.route('/updatebudgetstatus/<userId>', methods=['POST'])
# def updatebudgetstatus(userId):
#     # Client_name = flask.request.form['Client_name']
#     Package_price = flask.request.form['Package_price']
#     Amount_Paid = flask.request.form['Amount_Paid']
#     Pending_amount = flask.request.form['Pending_amount']
...
#     # insert
#     updateQuery = "UPDATE `budget` SET  `Package_price` = %s, `Amount_Paid` = %s, `Pending_amount` = %s, `Mode_of_payment` = %s, `Status` = %s WHERE `budget`.`Budget_id` = %s"
#     userInput = (Package_price,
#                  Amount_Paid, Pending_amount, Mode_of_payment, Status, userId)
...
# @app.route('/getexpertuser/<userId>', methods=['GET', 'POST', "OPTIONS"])
# def getexpertuser(userId):
#     cursor = mysql.connection.cursor()
#     # executing query
#     query = "SELECT * FROM expert WHERE Expert_id=%s"
#     userInput = (userId, )
...
#     # respp.set_data('Deleted!', result)
#     return respp
# @app.route('/updatetutors/<userId>', methods=['POST'])
# def updatetutors(userId):
#     print(userId)
#     Expert_firstname = flask.request.form['Expert_firstname']
#     Expert_lastname = flask.request.form['Expert_lastname']
#     Expert_email = flask.request.form['Expert_email']
...
#     updateQuery = "UPDATE `expert` SET `Expert_firstname` = %s, `Expert_lastname` = %s, `Expert_email` = %s, `Expert_contact` = %s, `Expert_Address` = %s WHERE `expert`.`Expert_id` =%s"
#     userInput = (Expert_firstname, Expert_lastname,
#                  Expert_email, Expert_contact, Expert_Address, userId)
#     result = cursor.execute(updateQuery, userInput)
#     mysql.connection.commit()
#     # select
#     cursor.execute("SELECT * FROM expert")
...
#         respp.set_data('failed')
#         return respp
# UPDATE `otm` SET `Otm_username` = 'change' WHERE `otm`.`Otm_ID` = 4;
